...
from datasets import  STATUS_TEST, STATUS_TRAIN,STATUS_TEST_NO_LABEL,STATUS_VAL, STATUS_TOY_TRAIN,STATUS_TOY_TEST#,pc_aug_funs
import pdb
...
class pc_data(Dataset):
    def __init__(self, pc_root, status='train', pc_input_num=1024):
        ...
        for name, _dir in names_dict.items():
            #classname = _dir.split("/")[-2]
            #modelname_ = _dir.split("/")[-1]
            #modelname = modelname_.split('_')[1]
            #modelname = modelname.replace('.npy', '')
    ...
    def __getitem__(self, idx):
        ...
        return torch.from_numpy(pc).float(), lbl,self.model_list[idx]
...
class view_data(Dataset):
    def __init__(self, view_root, base_model_name=models.ALEXNET, status=STATUS_TRAIN):
        ...
        for name, _dirs in names_dict.items():
            ...
            #classname = _dirs[0].split("/")[-2]
            modelname_ = _dirs[0].split("/")[-1]
            modelname = '_'.join(modelname_.split('_')[0:-1])
    ...
    def __getitem__(self, idx):
        ...
        return torch.stack(views).float(), self.lbl_list[idx], self.model_list[idx]
...
class pc_view_data(Dataset):
    def __init__(self, pc_root, view_root, base_model_name=models.ALEXNET, status='train', pc_input_num=1024):
        ...
        if status == STATUS_TRAIN:
            ...
            jpg_list = glob(osp.join(view_root, '*', 'test', '*.jpg')) #get all files for test
            npy_list = glob(osp.join(pc_root, '*', 'test', '*.npy'))
        ...
        for name in pc_dict.keys():
            ...
            self.lbl_list.append(name_list.index('_'.join(name.split('_')[:-1]))) #get the  label of the image
        self.view_num = len(self.view_list[0])
