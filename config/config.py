...
# device can be "cuda" or "gpu" # wrong it is not gpu chenq
device = 'cuda'
num_workers = 2
available_gpus = '1'
...
class view_net:
    ...
    shrec14_root=osp.join(work_root, 'data', 'shrec14/Projection_Shrec14')
    ...
    if base_model_name == (models.ALEXNET or models.RESNET50):
        ...
        ckpt_record_folder=ckpt_folder #directory for save records add by qchen119 12/25/2019
    else:
...
class pv_net:
    ...
    ckpt_load_file = osp.join(ckpt_folder, f'PVNet2-{base_model_name}-v94-ckpt.pth_')
